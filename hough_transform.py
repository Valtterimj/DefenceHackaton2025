...
"""
First version of the algoritm. Not part of the final product
...
"""
Create a simulated drone image from the satellite image
"""
...
crop = cv2.GaussianBlur(crop, (3,3), sigmaX=0.5) # add some blur to the image

"""
Display satellite and the drone image
"""
...
full_lines_vis = overlay_lines(full, lines_full)
crop_lines_vis = overlay_lines(crop, lines_crop)
...
"""
Create line masks and match the frames
"""
...
plt.figure(figsize=(12,5))
plt.subplot(1,2,1); plt.imshow(mask_full, cmap='gray'); plt.title("Satellite mask"); plt.axis("off")
plt.subplot(1,2,2); plt.imshow(mask_crop, cmap='gray'); plt.title("Drone mask"); plt.axis("off")
plt.tight_layout(); plt.show()
...
full_box_bgr = cv2.cvtColor(full, cv2.COLOR_GRAY2BGR)
